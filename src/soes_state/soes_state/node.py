...
class StateNode(Node):
    def __init__(self):
        ...
        # Hold/freeze bookkeeping: when swirl finishes we keep last IK joints fixed until higher-level logic proceeds
        self.hold_position = False
        self.hold_q: Optional[np.ndarray] = None
    ...
    def _arm_enter(self, new_phase: ArmPhase, xyz: Optional[np.ndarray]):
        ...
        if new_phase == ArmPhase.HOME:
            self._home_done_logged = False
            # entering HOME means we allow IK to move again -> clear any hold/freeze
            self.hold_position = False
            self.hold_q = None
    ...
    def _arm_tick(self):
        ...
        if self.arm_phase == ArmPhase.WAIT:
            # If hold is requested (freeze joint angles after swirl), keep publishing the last joint positions
            if self.hold_position and self.hold_q is not None:
                qdot_zero = np.zeros_like(self.hold_q)
                # publish hold position as position-mode (use_velocity=False)
                self._publish_targets(self.hold_q, qdot_zero, use_velocity=False)
                # indicate we are at target so higher-level code (INIT_POS settle) can work
        ...
        if self.arm_phase == ArmPhase.SWIRL and self.spiral_center is not None:
            ...
            if self.spiral_theta >= self.theta_max:
                ...
                # freeze current joint angles so we hold pose until higher-level logic proceeds
                self.hold_q = self.q.copy()
                self.hold_position = True
                # mark as at-target (start settle timer)
                self._set_arm_at(True)
                self.arm_at_since = self.get_clock().now()
                self._arm_enter(ArmPhase.WAIT, None)
                self._set_swirl_active(False)
