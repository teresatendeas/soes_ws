...
class I2CBridge(Node):
    ...
    def on_joint(self, msg: JointTargets):
        pos = [
            float(msg.position[i]) if i < len(msg.position) else 0.0
            for i in range(4)
        ]

        vel = [
            float(msg.velocity[i]) if hasattr(msg, 'velocity') and i < len(msg.velocity) else 0.0
            for i in range(4)
        ]
        ...
        if self.debug:
            self.get_logger().info(
                f'I2C 0x03 -> use_vel={use_velocity}, pos_s16={pos_s16}, vel_s16={vel_s16}'
            )
